...
def parse_requirements(filename):
    with open(filename, 'r') as f:
        lines = [line.strip() for line in f if line.strip() and not line.strip().startswith('#') and 'torch-scatter' not in line]
    return lines

def main():
    requirements_folder = 'requirements'
    requirements_files = [os.path.join(requirements_folder, f) for f in os.listdir(requirements_folder) if f.endswith('.txt')]
    ...
    install_requires.append(f"flash-attn @ https://github.com/Dao-AILab/flash-attention/releases/download/v{FLASH_ATTN_VERSION}/flash_attn-{FLASH_ATTN_VERSION}+cu{CUDA_VERSION}torch{TORCH_VERSION}cxx11abi{cxx11abi_flag}-{python_version}-{python_version}-{platform_name}.whl#egg=flash-attn=={FLASH_ATTN_VERSION}")    
